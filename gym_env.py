...
from utils import mse, trace_distance, compute_fidelity, mae
import gymnasium
...
class QuantumCircuit(gymnasium.Env):
    ...
    def __post_init__(self):
        ...
        self.only_diagonal = config["dataset"]["only_diagonal"]
        ...
        if self.circuits is None:
            ...
            if self.only_diagonal is True:
                print("ATTENZIONE: stai allenando i dati della diagonale")
                self.labels = np.array([np.diag(dm) for dm in self.labels])
            else:
                print("ATTENZIONE: stai allenando dati full")
